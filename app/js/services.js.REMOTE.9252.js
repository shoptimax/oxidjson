...
services.factory('Auth', function($http, $cookieStore, $cookies, $rootScope, $location) {
	return {
		 ...
		 */
		isLoggedIn : function() {
			$http.post('/shoptifind3/ee5/smxjson/action/login', "{}").success(function(response) {
		...
		login : function(user, success, error) {
			...
			$http.post('/shoptifind3/ee5/smxjson/action/login', jsonData, {
				headers : {
		...
		logout : function(success, error) {
			...
			var jsonData = '{}';
			$http.post('/shoptifind3/ee5/smxjson/action/logout', jsonData).success(function(response) {
				$rootScope.validLogin = false;
